...
class CameraModel:
    ...
    def setup_camera(self, camera_location=(-8, 0, 3), camera_type="sensor.camera.rgb"):
        ...
        camera_transform = carla.Transform(
            carla.Location(*camera_location), carla.Rotation(pitch=10)
        )

        # Attach camera to vehicle if provided, else spawn standalone
        if self.vehicle:
            self.camera = self.carla_model.world.spawn_actor(
    ...
    def change_camera_location(self, camera_location=(-15, 0, 3)):
        ...
        camera_transform = carla.Transform(
            carla.Location(*camera_location), carla.Rotation(pitch=15)
        )
