...
class TestActorCritic(unittest.TestCase):
    ...
    def test_actor_critic_init(self):
        ...
        self.assertTrue(torch.equal(self.model.cov_var, torch.full((self.action_dim,), self.action_std_init)))
        self.assertTrue(torch.equal(self.model.cov_mat, torch.diag(self.model.cov_var).unsqueeze(dim=0)))
    ...
    def test_actor_output_multiple(self):
        for _ in range(5):
            ...
            self.assertTrue(-1 <= action[0, 0].item() <= 1)  # Steer should be in [-1, 1]
