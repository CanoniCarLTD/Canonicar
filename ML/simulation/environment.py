...
class CarlaEnvironment:
    def __init__(
        self, client, world, town, checkpoint_frequency=100, continuous_action=True
    ) -> None:
        self.client = client
        self.world = world
    ...
    def reset(self):
        """return [image_obs, navigation_obs]"""
        try:
            ...
            WE NEED TO CHANGE IT FOR A FIXED SPAWN POINT (ETAI)
            ...
            """ MIGHT NEED TO CHANGE/REMOVE IT (ETAI) """
            ...
            """ STILL HAVEN'T READ THIS PART (ETAI) """
            if self.fresh_start:
                self.current_waypoint_index = 0
    ...
    def step(self, action_idx):
        """return [image_obs, navigation_obs], reward, done, [distance_covered, center_lane_deviation]"""
        try:
            self.timesteps += 1
            ...
            if self.continous_action_space:
                ...
                if self.velocity < 20.0:
                    self.vehicle.apply_control(
                        carla.VehicleControl(
                            steer=self.previous_steer * 0.9 + steer * 0.1, throttle=1.0
                        )
                    )
                else:
                    self.vehicle.apply_control(
    ...
    """ HERE WE NEED TO CHECK IF WE DO IT WITH FIXED ACTION VALUES """
    ...
    """ WE MAY WANNA CHANGE IT TO HAVE A FIXED COLOR (ETAI) """
    ...
    """ CHANGE IT TO HAVE A FIXED SPAWN POINT (ETAI) """
