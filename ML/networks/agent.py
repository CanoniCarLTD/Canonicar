...
class PPOAgent(object):
    ...
    def learn(self):
        ...
        old_states = (
            torch.squeeze(torch.stack(self.memory.observation, dim=0))
            .detach()
            .to(device)
        )
        old_actions = (
            torch.squeeze(torch.stack(self.memory.actions, dim=0)).detach().to(device)
        )
        old_logprobs = (
            torch.squeeze(torch.stack(self.memory.log_probs, dim=0)).detach().to(device)
        )
        ...
        for _ in range(self.n_updates_per_iteration):
            ...
            loss = (
                ...
                + 0.5 * self.MseLoss(values, rewards)
                - 0.01 * dist_entropy
            )
